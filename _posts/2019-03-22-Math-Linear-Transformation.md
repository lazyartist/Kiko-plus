...
$$
동차성의 \ 정의\\
임의의\ 수\ x와 \ a에\ 대해\ f(ax)=af(x)가\ 항상\ 성립
$$

- 동차성을 만족하려면 함수가 반드시 1차여야 한다.
...
1. 0이 아닌 임의의 수(w)를 정하고
2. 그 값을 x, y에 각각 곱한다.
3. 마지막 성분은 임의의 값(w)를 그대로 쓴다.

$$
w \ne 0\\
(x,y) \rightarrow (w \times x, w \times y, w)\\
\ \\
가장\ 간단한\ 형태는\ w가\ 1인\ 경우\\
...
1. 마지막 성분의 값으로 나머지 성분들을 각각 나눠준다.
2. 마지막 성분을 제외한 성분만 취한다.
